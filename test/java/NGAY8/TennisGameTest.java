...
public class TennisGameTest {
    ...
    public void checkAllScores() {
        int highestScore = Math.max(this.PLAYER1_SCORE, this.PLAYER2_SCORE);
        int m_score1 = 0;
        int m_score2 = 0;
        for (int i = 0; i < highestScore; i++) {
            if (i < this.PLAYER1_SCORE)
                m_score1 += 1;
            if (i < this.PLAYER2_SCORE)
                m_score2 += 1;
        }
        assertEquals(this.EXPECTED_SCORE, TennisGame.getScore("John", "Bill", m_score1, m_score2));
    }
}